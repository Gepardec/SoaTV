...
VisElement.builders["svg"] = {
	build : function(visElement, argsObject) {
		visElement.d3Container = d3.select("#" + argsObject.containerId)
				.append("svg").attr("width", argsObject.properties.svg.width)
...
VisElement.builders["node"] = {
	build : function(visElement, argsObject) {
		...
		var stripeRect = svg.append("rect").attr("x", x).attr("y",
				y - prop.node.topHeight - 5).attr("width", prop.node.width).attr(
				"height", prop.node.topHeight).attr(
				"style",
				"fill:" + prop.node[nodeType].topColor + ";stroke:"
						+ prop.node.borderColor + ";stroke-width:"
						+ prop.node.borderWidth + ";");

		var img = svg.append("image").attr("x", x + 5).attr("y", y - prop.node.topHeight).attr(
				"width", prop.node[nodeType].image.width).attr("height",
				prop.node[nodeType].image.height).attr("xlink:href",
				prop.node[nodeType].image.link);

		var text = svg.append("text").attr("x", x + 25).attr("y", y - prop.node.topHeight + 12).attr(
				"style", prop.node[nodeType].textStyle).text(nodeHeader);
...
VisElement.builders["component"] = {
	build : function(visElement, argsObject) {
		var componentType = argsObject.type;
		var nodeHeader = argsObject.header;
		var svg = visElement.globals.svg;
		...
		mainRect.attr("height", (1 + nrComponents) * prop.component.margin + nrComponents * prop.component.height);

		var x = parseInt(parentNode.d3Container.attr("x")) + prop.component.margin;
		var y = parseInt(parentNode.d3Container.attr("y")) + (nrComponents) * prop.component.margin + (nrComponents - 1) * prop.component.height;
		visElement.d3Container = svg.append("rect").attr("x", x).attr("y", y)
				/*.attr("rx", prop.component.borderRadius)*/.attr("width",
						prop.component.width).attr("height",
						prop.component.height).attr(
						"style",
...
VisElement.builders["message"] = {
	build : function(visElement, argsObject) {
		...
		grad.append("stop").attr("offset", "10%").attr("style",
				"stop-color:" + color + ";stop-opacity : 1");

		grad.append("stop") // TODO REMOVE THIS SHIT AFTER FINISHING
		.attr("offset", "100%").attr("style",
				"stop-color:white;stop-opacity : 0");*/
...
/**
 * Draws path on svg container with given parameters (interpolate, color, width, opacity) between p1 and p2
 */
...
/**
 * Draws line on svg container with given parameters (color, width, opacity) between p1 and p2
 */
